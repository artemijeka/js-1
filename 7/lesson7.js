...
var FIELD_SIZE_X = 22;//строки
var FIELD_SIZE_Y = 22;//столбцы
...
var scoreBox = document.querySelector('#snake-stats');

/* 
    * Убрать границы поля. Т.е., 
    при пересечении границы поля змейка появляется 
...
function startGame() {
    ...
    setTimeout(createFood(true), 5000);
}
...
/**
 * Создание еды и препятствий
 */
function createFood(block = false) {
    if (block === false) {
    ...
    if (block === true) {
        ...
        while (!blockCreated) { //пока еду не создали
            // рандом
            ...
            if (!block_cell_classes.includes('snake-unit')) {
                ...
                for (var i = 0; i < block_cell_classes.length; i++) {
                    classesBlock += block_cell_classes[i] + ' ';
                }
                block_cell.setAttribute('class', classesBlock + 'block-unit');
                blockCreated = true;
            }
        }
    }
